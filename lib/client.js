...
class Client {
   ...
   */
  listClass(version, orgUnitId, userId = this._userId, userKey = this._userKey) {
    this._setUserContext(userId, userKey);
   ...
   * @param {String} [userIdOfRequestingUser] - id of the requesting user
   * @param {String} [userKey] - key of the requesting user
   * @return {Promise}
   */
  getUser(version, userId, userIdOfRequestingUser = this._userId, userKey = this._userKey) {
    this._setUserContext(userIdOfRequestingUser, userKey);
  ...
  listEnrollments(version, orgUnitId, userId = this._userId, userKey = this._userKey) {
    ...
    return this
      .listClass(version, orgUnitId, userId, userKey)
      .then(function getUserDetails(enrollmentList) {
        let promises = [];

        //get details of each user
        enrollmentList.forEach(function (enrollment) {
          promises.push(_this.getUser(version, enrollment.Identifier, userId, userKey));
        });
